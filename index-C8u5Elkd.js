...
const S = "https://www.themealdb.com/api/json/v1/1";
async function U(s) {
    try {
        return (await (await fetch(`${S}/search.php?s=${encodeURIComponent(s)}`)).json()).meals || []
    } catch (e) {
        return console.error("Error searching meals by name:", e),
        []
    }
}
async function V(s) {
    try {
        return (await (await fetch(`${S}/search.php?f=${s}`)).json()).meals || []
    } catch (e) {
        return console.error("Error searching meals by letter:", e),
        []
    }
}
async function z(s) {
    try {
        return (await (await fetch(`${S}/filter.php?i=${encodeURIComponent(s)}`)).json()).meals || []
    } catch (e) {
        return console.error("Error filtering meals by ingredient:", e),
        []
    }
}
async function W(s) {
    try {
        return (await (await fetch(`${S}/filter.php?c=${encodeURIComponent(s)}`)).json()).meals || []
    } catch (e) {
        return console.error("Error filtering meals by category:", e),
        []
    }
}
async function Y(s) {
    try {
        return (await (await fetch(`${S}/filter.php?a=${encodeURIComponent(s)}`)).json()).meals || []
    } catch (e) {
        return console.error("Error filtering meals by area:", e),
        []
    }
}
async function J() {
    try {
        return (await (await fetch(`${S}/categories.php`)).json()).categories || []
    } catch (s) {
        return console.error("Error fetching categories:", s),
        []
    }
}
async function K() {
    try {
        return (await (await fetch(`${S}/list.php?c=list`)).json()).meals || []
    } catch (s) {
        return console.error("Error fetching category list:", s),
        []
    }
}
async function Q() {
    try {
        return (await (await fetch(`${S}/list.php?a=list`)).json()).meals || []
    } catch (s) {
        return console.error("Error fetching area list:", s),
        []
    }
}
async function X() {
    try {
        return (await (await fetch(`${S}/list.php?i=list`)).json()).meals || []
    } catch (s) {
        return console.error("Error fetching ingredient list:", s),
        []
    }
}
async function Z(s) {
    try {
        const t = await (await fetch(`${S}/lookup.php?i=${s}`)).json();
        return t.meals ? t.meals[0] : null
    } catch (e) {
        return console.error("Error fetching meal by ID:", e),
        null
    }
}
async function j() {
    try {
        const e = await (await fetch(`${S}/random.php`)).json();
        return e.meals ? e.meals[0] : null
    } catch (s) {
        return console.error("Error fetching random meal:", s),
        null
    }
}
async function ee(s=5) {
    try {
        const e = Array(s).fill().map( () => j());
        return (await Promise.all(e)).filter(a => a !== null)
    } catch (e) {
        return console.error("Error fetching multiple random meals:", e),
        []
    }
}
function te(s) {
...
function ae(s, e="small") {
    const t = e === "medium" ? "-medium" : "-small";
    return `https://www.themealdb.com/images/ingredients/${encodeURIComponent(s)}${t}.png`
}
function se(s) {
    return s ? s.split(/(?:\r\n|\r|\n)+/).map(t => t.trim()).filter(t => t.length > 0).map(t => t.replace(/^\d+[\.\)]\s*/, "")).filter(t => (t.toLowerCase(),
    /^step\s*\d+\.?$/i.test(t) || /^\d+\.?$/.test(t) ? !1 : t.length > 5)) : []
}
const x = {
    searchMealsByName: U,
    searchMealsByFirstLetter: V,
    filterMealsByIngredient: z,
    filterMealsByCategory: W,
    filterMealsByArea: Y,
    getAllCategories: J,
    getCategoryList: K,
    getAreaList: Q,
    ...
    getMealById: Z,
    getRandomMeal: j,
    getMultipleRandomMeals: ee,
    extractIngredients: te,
    getIngredientThumbnail: ae,
    parseInstructions: se
}
  , q = "https://nutriplan-api.vercel.app/api"
...
async function G(s, e) {
    var a, r;
    const t = `recipe_${s}_${e.join("|")}`;
    if (T.has(t))
        return T.get(t);
    try {
        const o = await fetch(`${q}/nutrition/analyze`, {
            method: "POST",
            headers: {
        ...
        if (!o.ok) {
            ...
            throw console.error("❌ Nutrition API error:", d),
            new Error(((a = d.error) == null ? void 0 : a.message) || `API error: ${o.status}`)
        }
        const i = await o.json();
        if (!i.success)
            throw console.error("❌ API returned failure:", i),
            new Error(((r = i.error) == null ? void 0 : r.message) || i.error || "Analysis failed");
        const l = i.data
          , n = {
            ...
            cautions: [],
            totals: l.totals,
            perServing: l.perServing,
            totalNutrients: {
                ENERC_KCAL: {
                    label: "Energy",
                    quantity: l.totals.calories,
                    unit: "kcal"
                },
                FAT: {
                    label: "Fat",
                    quantity: l.totals.fat,
                    unit: "g"
                },
                FASAT: {
                    label: "Saturated Fat",
                    quantity: l.totals.saturatedFat,
                    unit: "g"
                },
                CHOCDF: {
                    label: "Carbohydrates",
                    quantity: l.totals.carbs,
                    unit: "g"
                },
                FIBTG: {
                    label: "Fiber",
                    quantity: l.totals.fiber,
                    unit: "g"
                },
                SUGAR: {
                    label: "Sugars",
                    quantity: l.totals.sugar,
                    unit: "g"
                },
                PROCNT: {
                    label: "Protein",
                    quantity: l.totals.protein,
                    unit: "g"
                },
                CHOLE: {
                    label: "Cholesterol",
                    quantity: l.totals.cholesterol,
                    unit: "mg"
                },
                NA: {
                    label: "Sodium",
                    quantity: l.totals.sodium,
                    unit: "mg"
                }
            },
            totalDaily: oe(l.totals),
            ingredients: l.ingredients.map(d => {
                var u, m, p, f, b, v;
                return {
                    text: d.original,
                    food: ((u = d.matched) == null ? void 0 : u.description) || ((m = d.parsed) == null ? void 0 : m.foodName),
                    grams: d.grams,
                    calories: ((p = d.nutrition) == null ? void 0 : p.calories) || 0,
                    protein: ((f = d.nutrition) == null ? void 0 : f.protein) || 0,
                    fat: ((b = d.nutrition) == null ? void 0 : b.fat) || 0,
                    carbs: ((v = d.nutrition) == null ? void 0 : v.carbs) || 0
                }
            }
            )
        };
        ...
        return console.error("❌ Error analyzing recipe:", o),
        ie(s, e)
    }
}
function oe(s) {
    ...
    return {
        ENERC_KCAL: {
            label: "Energy",
            quantity: Math.round(s.calories / e.calories * 100),
            unit: "%"
        },
        FAT: {
            label: "Fat",
            quantity: Math.round(s.fat / e.fat * 100),
            unit: "%"
        },
        FASAT: {
            label: "Saturated Fat",
            quantity: Math.round(s.saturatedFat / e.saturatedFat * 100),
            unit: "%"
        },
        CHOCDF: {
            label: "Carbohydrates",
            quantity: Math.round(s.carbs / e.carbs * 100),
            unit: "%"
        },
        FIBTG: {
            label: "Fiber",
            quantity: Math.round(s.fiber / e.fiber * 100),
            unit: "%"
        },
        PROCNT: {
            label: "Protein",
            quantity: Math.round(s.protein / e.protein * 100),
            unit: "%"
        },
        CHOLE: {
            label: "Cholesterol",
            quantity: Math.round(s.cholesterol / e.cholesterol * 100),
            unit: "%"
        },
        NA: {
            label: "Sodium",
            quantity: Math.round(s.sodium / e.sodium * 100),
            unit: "%"
        }
    }
}
    ...
    if (t && a)
        return {
            ...
            macros: {
                protein: {
                    amount: t.protein,
                    dailyValue: Math.round(t.protein / 50 * 100)
                },
                carbs: {
                    amount: t.carbs,
                    dailyValue: Math.round(t.carbs / 300 * 100)
                },
                fat: {
                    amount: t.fat,
                    dailyValue: Math.round(t.fat / 65 * 100)
                },
                fiber: {
                    amount: t.fiber,
                    dailyValue: Math.round(t.fiber / 25 * 100)
                },
                sugar: {
                    amount: t.sugar,
                    dailyValue: 0
                ...
                saturatedFat: {
                    amount: t.saturatedFat,
                    dailyValue: Math.round(t.saturatedFat / 20 * 100)
                }
            },
            other: {
            ...
            dietLabels: s.dietLabels || [],
            healthLabels: s.healthLabels || []
        };
    const r = s.totalNutrients || {}
      , o = s.totalDaily || {};
    return {
        servings: e,
        caloriesPerServing: Math.round((s.calories || 0) / e),
        totalCalories: Math.round(s.calories || 0),
        macros: {
            protein: {
                amount: Math.round((((i = r.PROCNT) == null ? void 0 : i.quantity) || 0) / e),
                dailyValue: Math.round((((l = o.PROCNT) == null ? void 0 : l.quantity) || 0) / e)
            },
            carbs: {
                amount: Math.round((((n = r.CHOCDF) == null ? void 0 : n.quantity) || 0) / e),
                dailyValue: Math.round((((d = o.CHOCDF) == null ? void 0 : d.quantity) || 0) / e)
            },
            fat: {
                amount: Math.round((((u = r.FAT) == null ? void 0 : u.quantity) || 0) / e),
                dailyValue: Math.round((((m = o.FAT) == null ? void 0 : m.quantity) || 0) / e)
            },
            fiber: {
                amount: Math.round((((p = r.FIBTG) == null ? void 0 : p.quantity) || 0) / e),
                dailyValue: Math.round((((f = o.FIBTG) == null ? void 0 : f.quantity) || 0) / e)
            },
            sugar: {
                amount: Math.round((((b = r.SUGAR) == null ? void 0 : b.quantity) || 0) / e),
                dailyValue: 0
            },
            saturatedFat: {
                amount: Math.round((((v = r.FASAT) == null ? void 0 : v.quantity) || 0) / e),
                dailyValue: Math.round((((y = o.FASAT) == null ? void 0 : y.quantity) || 0) / e)
            }
        },
        other: {
            cholesterol: Math.round(((($ = r.CHOLE) == null ? void 0 : $.quantity) || 0) / e),
            sodium: Math.round((((w = r.NA) == null ? void 0 : w.quantity) || 0) / e)
        },
        dietLabels: s.dietLabels || [],
        healthLabels: s.healthLabels || []
    }
...
function ne(s=[]) {
    ...
    for (const t of s)
        t.nutrition && (e.calories += t.nutrition.calories || 0,
        e.protein += t.nutrition.protein || 0,
        e.carbs += t.nutrition.carbs || 0,
        e.fat += t.nutrition.fat || 0,
        e.fiber += t.nutrition.fiber || 0);
    return e
}
async function de(s) {
    const e = await G("Single Item", [s]);
    if (e.ingredients && e.ingredients.length > 0) {
        const t = e.ingredients[0];
        return {
...
async function ce(s, e=5) {
    try {
        const t = await fetch(`${q}/nutrition/search?q=${encodeURIComponent(s)}&page=1`, {
            headers: {
                "x-api-key": D
            }
        });
        if (!t.ok)
            throw new Error(`Search API error: ${t.status}`);
        return (await t.json()).results || []
    } catch (t) {
        return console.error("Error searching foods:", t),
        []
    }
}
const A = {
    analyzeRecipe: G,
    formatNutritionForDisplay: le,
    calculateDayTotal: ne,
    getNutritionForItem: de,
    searchFoods: ce,
    clearNutritionCache: re
}
  , N = "https://world.openfoodfacts.org";
async function ge(s={}) {
    try {
        const e = new URLSearchParams({
            page: s.page || 1,
            page_size: s.pageSize || 24,
            json: 1,
            ...s.searchTerms && {
                search_terms: s.searchTerms
            },
            ...s.categories && {
                categories_tags_en: s.categories
            },
            ...s.nutritionGrade && {
                nutrition_grades_tags: s.nutritionGrade
            }
        })
          , t = await fetch(`${N}/cgi/search.pl?${e}`);
        if (!t.ok)
        ...
        return {
            count: a.count || 0,
            page: a.page || 1,
            pageSize: a.page_size || 24,
            products: (a.products || []).map(F)
        }
    } catch (e) {
        return console.error("Error searching products:", e),
        xe(s)
    }
}
async function ue(s) {
    try {
        const e = await fetch(`${N}/api/v0/product/${s}.json`);
        if (!e.ok)
            throw new Error(`HTTP error! status: ${e.status}`);
        const t = await e.json();
        return t.status === 0 ? null : F(t.product)
    } catch (e) {
        return console.error("Error fetching product by barcode:", e),
        null
    }
}
async function me(s, e=1, t=24) {
    try {
        const a = await fetch(`${N}/category/${encodeURIComponent(s)}.json?page=${e}&page_size=${t}`);
        if (!a.ok)
        ...
        return {
            count: r.count || 0,
            page: r.page || 1,
            products: (r.products || []).map(F)
        }
    } catch (a) {
        return console.error("Error fetching products by category:", a),
        {
            count: 0,
            page: 1,
            products: []
...
function F(s) {
    var e, t, a, r, o, i, l, n, d, u;
    return {
        barcode: s.code || s._id,
        name: s.product_name || s.product_name_en || "Unknown Product",
        brand: s.brands || "",
        categories: s.categories || "",
        image: s.image_front_url || s.image_url || null,
        thumbnailImage: s.image_front_small_url || s.image_small_url || null,
        nutritionGrade: s.nutrition_grades || s.nutrition_grade_fr || null,
        novaGroup: s.nova_group || null,
        ecoscore: s.ecoscore_grade || null,
        ingredients: s.ingredients_text || s.ingredients_text_en || "",
        allergens: s.allergens || "",
        quantity: s.quantity || "",
        servingSize: s.serving_size || "",
        nutrition: {
            calories: ((e = s.nutriments) == null ? void 0 : e["energy-kcal_100g"]) || ((t = s.nutriments) == null ? void 0 : t.energy_100g) || 0,
            fat: ((a = s.nutriments) == null ? void 0 : a.fat_100g) || 0,
            saturatedFat: ((r = s.nutriments) == null ? void 0 : r["saturated-fat_100g"]) || 0,
            carbs: ((o = s.nutriments) == null ? void 0 : o.carbohydrates_100g) || 0,
            sugar: ((i = s.nutriments) == null ? void 0 : i.sugars_100g) || 0,
            fiber: ((l = s.nutriments) == null ? void 0 : l.fiber_100g) || 0,
            protein: ((n = s.nutriments) == null ? void 0 : n.proteins_100g) || 0,
            salt: ((d = s.nutriments) == null ? void 0 : d.salt_100g) || 0,
            sodium: ((u = s.nutriments) == null ? void 0 : u.sodium_100g) || 0
        },
        labels: s.labels || "",
        origins: s.origins || "",
        stores: s.stores || ""
...
function ye(s, e=100) {
    const t = e / 100
      , a = s.nutrition;
    return {
        calories: Math.round(a.calories * t),
        fat: Math.round(a.fat * t * 10) / 10,
        saturatedFat: Math.round(a.saturatedFat * t * 10) / 10,
        carbs: Math.round(a.carbs * t * 10) / 10,
        sugar: Math.round(a.sugar * t * 10) / 10,
        fiber: Math.round(a.fiber * t * 10) / 10,
        protein: Math.round(a.protein * t * 10) / 10,
        salt: Math.round(a.salt * t * 100) / 100,
        sodium: Math.round(a.sodium * t)
    }
}
function xe(s={}) {
    let t = [{
    ...
    if (s.searchTerms) {
        const a = s.searchTerms.toLowerCase();
        t = t.filter(r => r.product_name.toLowerCase().includes(a) || r.brands.toLowerCase().includes(a))
    }
    return s.nutritionGrade && (t = t.filter(a => a.nutrition_grades === s.nutritionGrade.toLowerCase())),
    {
        count: t.length,
        page: s.page || 1,
...
const k = {
    searchProducts: ge,
    getProductByBarcode: ue,
    getProductsByCategory: me,
    getPopularCategories: pe,
    getNutriScoreInfo: fe,
    getNovaGroupInfo: be,
    calculateNutritionPerServing: ye
}
  , M = {
...
function ve() {
    const s = localStorage.getItem(M.USER_SETTINGS);
    c.userSettings = s ? JSON.parse(s) : {
        ...he
    };
    const e = localStorage.getItem(M.SAVED_RECIPES);
    c.savedRecipes = e ? JSON.parse(e) : [];
    const t = localStorage.getItem(M.DAILY_LOG);
    c.dailyLog = t ? JSON.parse(t) : {};
    const a = localStorage.getItem(M.SHOPPING_LIST);
    return c.shoppingList = a ? JSON.parse(a) : [],
    c.streaks = we(c.dailyLog),
    c
}
function we(s) {
    const e = new Date;
...
function L(s, e=!1) {
    Object.assign(c, s),
    e && (s.savedRecipes !== void 0 && localStorage.setItem(M.SAVED_RECIPES, JSON.stringify(c.savedRecipes)),
    s.dailyLog !== void 0 && localStorage.setItem(M.DAILY_LOG, JSON.stringify(c.dailyLog)),
    s.userSettings !== void 0 && localStorage.setItem(M.USER_SETTINGS, JSON.stringify(c.userSettings)),
    s.shoppingList !== void 0 && localStorage.setItem(M.SHOPPING_LIST, JSON.stringify(c.shoppingList))),
    window.dispatchEvent(new CustomEvent("stateChange",{
        detail: s
    }))
}
function $e(s) {
    c.savedRecipes.some(t => t.idMeal === s.idMeal) || (c.savedRecipes.push({
        ...s,
        savedAt: new Date().toISOString()
    }),
    L({
        savedRecipes: c.savedRecipes
    }, !0))
}
function Le(s) {
    c.savedRecipes = c.savedRecipes.filter(e => e.idMeal !== s),
    L({
        savedRecipes: c.savedRecipes
    }, !0)
}
function Ce(s) {
    return c.savedRecipes.some(e => e.idMeal === s)
}
function Me(s, e) {
    c.dailyLog[s] || (c.dailyLog[s] = {
        meals: [],
        totalCalories: 0,
        totalProtein: 0,
        totalCarbs: 0,
    ...
    c.dailyLog[s].meals.push(e),
    c.dailyLog[s].totalCalories += e.calories || 0,
    c.dailyLog[s].totalProtein += e.protein || 0,
    c.dailyLog[s].totalCarbs += e.carbs || 0,
    c.dailyLog[s].totalFat += e.fat || 0,
    L({
        dailyLog: c.dailyLog
    }, !0)
}
function R(s, e) {
    ...
    c.dailyLog[s].water += e,
    c.dailyLog[s].waterLog = c.dailyLog[s].waterLog || [],
    c.dailyLog[s].waterLog.push({
        amount: e,
...
function O() {
    const s = B()
      ...
      , t = c.userSettings.waterGoal
      , a = c.userSettings.waterGlassSize;
    return {
        current: e.water || 0,
        goal: t,
        glassSize: a,
        glasses: Math.floor((e.water || 0) / a),
        targetGlasses: Math.ceil(t / a),
        percentage: Math.min(100, Math.round((e.water || 0) / t * 100)),
        log: e.waterLog || []
    }
}
function ke() {
    const s = B()
      , e = c.userSettings.waterGlassSize;
    return R(s, e),
    O()
}
function Ee(s) {
    const e = c.dailyLog[s] || {
        totalCalories: 0,
        totalProtein: 0,
        totalCarbs: 0,
        totalFat: 0,
      ...
      , t = c.userSettings;
    return {
        calories: Math.min(100, Math.round(e.totalCalories / t.calorieGoal * 100)),
        protein: Math.min(100, Math.round(e.totalProtein / t.proteinGoal * 100)),
        carbs: Math.min(100, Math.round(e.totalCarbs / t.carbsGoal * 100)),
        fat: Math.min(100, Math.round(e.totalFat / t.fatGoal * 100)),
        water: Math.min(100, Math.round(e.water / t.waterGoal * 100)),
        overall: 0
    }
}
function Ie(s) {
    s.forEach(e => {
        c.shoppingList.some(a => a.ingredient.toLowerCase() === e.ingredient.toLowerCase()) || c.shoppingList.push({
            ...e,
...
function Pe(s) {
    const e = c.shoppingList.find(t => t.id === s);
...
function Te(s) {
    c.shoppingList = c.shoppingList.filter(e => e.id !== s),
    L({
        shoppingList: c.shoppingList
    }, !0)
}
function Fe() {
    c.shoppingList = c.shoppingList.filter(s => !s.checked),
    L({
        shoppingList: c.shoppingList
    }, !0)
}
...
function Be() {
    ...
    for (let t = 6; t >= 0; t--) {
        ...
        e.push(a.toISOString().split("T")[0])
    }
    return e.map(t => ({
        date: t,
        dayName: new Date(t).toLocaleDateString("en-US", {
            weekday: "short"
        }),
        nutrition: c.dailyLog[t] || {
            totalCalories: 0
        }
    }))
}
function _e() {
    const {weight: s, height: e} = c.userSettings;
    if (!s || !e)
        return null;
    const t = e / 100
      , a = s / (t * t);
    let r = "Normal";
    return a < 18.5 ? r = "Underweight" : a >= 25 && a < 30 ? r = "Overweight" : a >= 30 && (r = "Obese"),
    {
        value: a.toFixed(1),
        category: r
    }
...
function Ae() {
    var r, o;
    const s = ((r = c.savedRecipes) == null ? void 0 : r.length) || 0
      , e = Object.values(c.mealPlan || {}).reduce( (i, l) => i + Object.keys(l).length, 0)
      , t = ((o = c.shoppingList) == null ? void 0 : o.length) || 0
      , a = Object.keys(c.workoutLog || {}).length;
    return {
        savedRecipes: s,
        plannedMeals: e,
        shoppingItems: t,
...
const g = {
    ...
    unsaveRecipe: Le,
    isRecipeSaved: Ce,
    logDailyNutrition: Me,
    logWaterIntake: R,
    getTodayWaterIntake: O,
    logWaterGlass: ke,
    getDailyProgress: Ee,
    addToShoppingList: Ie,
    toggleShoppingItem: Pe,
    removeFromShoppingList: Te,
    clearCompletedShoppingItems: Fe,
...
function qe(s) {
    const e = {
      ...
      , t = e[s.strCategory] || e.Miscellaneous
      , r = {
...
function De(s, e, t, a) {
    var i, l, n, d, u, m, p, f, b, v, y, $, w, E, I, P, _;
    const r = (e == null ? void 0 : e.dietLabels) || []
      , o = ((i = e == null ? void 0 : e.healthLabels) == null ? void 0 : i.slice(0, 5)) || [];
    return `
...
const h = {
    createMealCard: je,
    createCategoryCard: qe,
    createMealDetailContent: De,
    createLoadingSpinner: Ge,
    createEmptyState: Re,
    createAreaFilters: Oe,
    createDashboardWidget: He,
    createWaterTracker: Ue,
    createQuickActionCard: Ve,
    createSettingsSection: ze,
    createStreakCard: We,
    createSkeletonCard: Ye,
    createProductCard: Je,
    createProductDetailContent: Ke,
    createProductCategoryButton: Qe
};
function He(s, e, t, a, r="emerald", o=null) {
    const i = o ? `
...
function Ue(s) {
    const {current: e, goal: t, glasses: a, targetGlasses: r, percentage: o} = s
      , i = Array(r).fill(0).map( (l, n) => `
...
function Je(s) {
    ...
    const e = {
      ...
      , a = e[(o = s.nutritionGrade) == null ? void 0 : o.toLowerCase()] || "bg-gray-400"
      , r = t[s.novaGroup] || "bg-gray-400";
    return `
...
function Ke(s, e, t) {
    var a, r, o, i, l, n, d, u, m, p, f, b, v, y, $, w, E, I, P;
    return `
        <div class="p-6">
            <!-- Header -->
...
function Qe(s) {
    return `
        <button class="product-category-btn flex-shrink-0 px-5 py-3 bg-gradient-to-r ${{
    ...
    }[s.id] || "from-gray-500 to-gray-600"} text-white rounded-xl font-semibold hover:shadow-lg transition-all" data-category="${s.id}">
...
class Xe {
    ...
    async init() {
        ...
        this.setupRouting(),
        (window.location.pathname === "/" || window.location.pathname === "") && window.history.replaceState({
            page: "meals"
        }, "", "/home"),
        await this.loadInitialData();
        const e = this.getPageFromURL();
        e.type === "meal-detail" && e.slug ? await this.loadMealFromSlug(e.slug) : (this.renderPage(e.type),
        this.updateActiveNavLink(e.type)),
        this.hideLoadingOverlay()
    }
    setupRouting() {
        window.addEventListener("popstate", () => {
            const e = this.getPageFromURL();
            e.type === "meal-detail" ? this.loadMealFromSlug(e.slug) : (this.renderPage(e.type),
            this.updateActiveNavLink(e.type))
        }
        )
    }
    getPageFromURL() {
        const e = window.location.pathname.replace(/^\//, "").replace(/\/$/, "");
        return e.startsWith("meal/") ? {
            type: "meal-detail",
            slug: e.replace("meal/", "")
        } : {
            type: this.routes[e] || "meals",
            slug: null
        }
    }
    async loadMealFromSlug(e) {
        try {
            const t = e.replace(/-/g, " ")
              , a = await x.searchMeals(t);
            if (a && a.length > 0) {
                const r = a.find(o => this.slugify(o.strMeal) === e) || a[0];
                g.updateState({
                    selectedMealId: r.idMeal
                }),
            ...
            console.error("Error loading meal from URL:", t),
            this.navigateTo("meals")
        }
    }
    slugify(e) {
    ...
    navigateTo(e) {
        let t;
        e === "meals" ? t = "/home" : t = `/${Object.keys(this.routes).find(r => this.routes[r] === e && r !== "" && r !== "home") || e}`,
        window.location.pathname !== t && window.history.pushState({
            page: e
        }, "", t),
        this.renderPage(e),
        this.updateActiveNavLink(e)
    }
    navigateToMeal(e) {
        const a = `/meal/${this.slugify(e.strMeal)}`;
        g.updateState({
            selectedMealId: e.idMeal
        }),
    ...
    updateActiveNavLink(e) {
        document.querySelectorAll("#sidebar nav a").forEach(t => {
            var o, i, l, n, d, u;
            const a = ((i = (o = t.querySelector("span")) == null ? void 0 : o.textContent) == null ? void 0 : i.toLowerCase()) || "";
            let r = "meals";
            a.includes("meals") || a.includes("recipes") ? r = "meals" : a.includes("settings") ? r = "settings" : a.includes("products") || a.includes("barcode") || a.includes("scan") ? r = "products" : (a.includes("food log") || a.includes("log")) && (r = "foodlog"),
            r === e ? (t.classList.add("bg-emerald-50", "text-emerald-700"),
            t.classList.remove("text-gray-600", "hover:bg-gray-50"),
            (l = t.querySelector("span")) == null || l.classList.add("font-semibold"),
            (n = t.querySelector("span")) == null || n.classList.remove("font-medium")) : (t.classList.remove("bg-emerald-50", "text-emerald-700"),
            t.classList.add("text-gray-600", "hover:bg-gray-50"),
            (d = t.querySelector("span")) == null || d.classList.remove("font-semibold"),
            (u = t.querySelector("span")) == null || u.classList.add("font-medium"))
        }
        )
    }
    hideLoadingOverlay() {
        const e = document.getElementById("app-loading-overlay");
        e && (e.style.opacity = "0",
    ...
    setupEventListeners() {
        document.querySelectorAll("#sidebar nav a").forEach(t => {
            t.addEventListener("click", a => this.handleNavigation(a))
        }
        );
        const e = document.querySelector('#search-filters-section input[type="text"]');
        e && (e.addEventListener("input", t => this.handleSearch(t)),
        e.addEventListener("keypress", t => {
            t.key === "Enter" && this.performSearch(t.target.value)
        }
        )),
        this.setupViewToggle(),
        document.addEventListener("click", t => this.handleGlobalClick(t)),
        window.addEventListener("stateChange", t => this.handleStateChange(t))
    }
    setupViewToggle() {
        const e = document.getElementById("grid-view-btn")
          , t = document.getElementById("list-view-btn");
        e && t && (e.addEventListener("click", () => this.setViewMode("grid")),
        t.addEventListener("click", () => this.setViewMode("list")))
    }
    setViewMode(e) {
        var o, i, l, n;
        const t = document.getElementById("grid-view-btn")
          , a = document.getElementById("list-view-btn")
          , r = document.querySelector("#all-recipes-section .grid");
        r && (e === "grid" ? (t == null || t.classList.add("bg-white", "shadow-sm"),
        (o = t == null ? void 0 : t.querySelector("i")) == null || o.classList.replace("text-gray-500", "text-gray-700"),
        a == null || a.classList.remove("bg-white", "shadow-sm"),
        (i = a == null ? void 0 : a.querySelector("i")) == null || i.classList.replace("text-gray-700", "text-gray-500"),
        r.className = "grid grid-cols-4 gap-5",
        r.querySelectorAll(".recipe-card").forEach(d => {
            var m, p, f, b;
            d.classList.remove("flex", "flex-row", "h-40"),
            (m = d.querySelector(".relative")) == null || m.classList.remove("w-48", "h-full"),
            (p = d.querySelector(".relative")) == null || p.classList.add("h-48"),
            (f = d.querySelector("img")) == null || f.classList.remove("h-full"),
            (b = d.querySelector("img")) == null || b.classList.add("h-full");
            const u = d.querySelector(".relative > .absolute.bottom-3");
            u && u.classList.remove("hidden")
        }
        )) : (a == null || a.classList.add("bg-white", "shadow-sm"),
        (l = a == null ? void 0 : a.querySelector("i")) == null || l.classList.replace("text-gray-500", "text-gray-700"),
        t == null || t.classList.remove("bg-white", "shadow-sm"),
        (n = t == null ? void 0 : t.querySelector("i")) == null || n.classList.replace("text-gray-700", "text-gray-500"),
        r.className = "grid grid-cols-2 gap-4",
        r.querySelectorAll(".recipe-card").forEach(d => {
            var m, p;
            d.classList.add("flex", "flex-row", "h-40"),
            (m = d.querySelector(".relative")) == null || m.classList.add("w-48", "h-full"),
            (p = d.querySelector(".relative")) == null || p.classList.remove("h-48");
            const u = d.querySelector(".relative > .absolute.bottom-3");
    ...
    handleNavigation(e) {
        var o, i;
        e.preventDefault();
        const a = ((i = (o = e.currentTarget.querySelector("span")) == null ? void 0 : o.textContent) == null ? void 0 : i.toLowerCase()) || "";
        let r = "meals";
        a.includes("meals") || a.includes("recipes") ? r = "meals" : a.includes("settings") ? r = "settings" : a.includes("products") || a.includes("barcode") || a.includes("scan") ? r = "products" : (a.includes("food log") || a.includes("log")) && (r = "foodlog"),
        this.navigateTo(r)
    }
    handleGlobalClick(e) {
        if (e.target.closest(".recipe-card")) {
            const a = e.target.closest(".recipe-card").dataset.mealId;
            this.showMealDetail(a)
        }
        if (e.target.closest(".category-card")) {
            const a = e.target.closest(".category-card").dataset.category;
            this.filterByCategory(a)
        }
        if (e.target.closest(".area-filter-btn")) {
            const a = e.target.closest(".area-filter-btn").dataset.area;
            this.filterByArea(a)
        }
        if (e.target.closest(".exercise-card") && !e.target.closest(".add-exercise-btn")) {
            const a = e.target.closest(".exercise-card").dataset.exerciseId;
            this.showExerciseDetail(a)
        }
        if (e.target.closest(".add-exercise-btn")) {
            e.stopPropagation();
            const a = e.target.closest(".exercise-card").dataset.exerciseId;
            this.addExerciseToWorkout(a)
        }
        if (e.target.closest(".close-detail-btn") && this.closeMealDetail(),
        e.target.closest(".add-to-plan-btn")) {
            const a = e.target.closest(".add-to-plan-btn").dataset.mealId;
            this.showMealPlanModal(a)
        }
    }
    handleSearch(e) {
        ...
        this.debounceTimer = setTimeout( () => {
            t.length >= 2 ? this.performSearch(t) : t.length === 0 && this.loadAllRecipes()
        }
        , 300)
    }
    async performSearch(e) {
        ...
        const t = document.querySelector("#all-recipes-section .grid");
        t && (t.innerHTML = h.createLoadingSpinner());
        try {
            const a = await x.searchMealsByName(e);
            g.updateState({
                meals: a,
                isLoading: !1
            }),
            this.renderRecipeGrid(a);
            const r = document.querySelector("#all-recipes-section p.text-gray-600");
            r && (r.textContent = `Showing ${a.length} recipes for "${e}"`)
        } catch (a) {
    ...
    async loadInitialData() {
        try {
            const e = await x.getAllCategories();
            g.updateState({
                categories: e
            });
            const t = await x.getAreaList();
            g.updateState({
                areas: t
            });
            const a = await x.searchMealsByName("chicken");
            g.updateState({
                meals: a
            })
        } catch (e) {
    ...
    async loadAllRecipes() {
        const e = await x.searchMealsByName("");
        if (e.length === 0) {
            const t = await x.searchMealsByName("chicken");
            g.updateState({
                meals: t
            }),
            this.renderRecipeGrid(t)
        } else
            g.updateState({
                meals: e
            }),
            this.renderRecipeGrid(e)
    }
    renderPage(e) {
        this.currentPage = e;
        const t = document.getElementById("main-content");
        switch (this.updateHeader(e),
        ["shopping-section", "settings-section", "products-section", "meal-detail-section", "foodlog-section"].forEach(a => {
    ...
    updateHeader(e) {
        ...
        t && r[e] && (t.textContent = r[e].title),
        a && r[e] && (a.textContent = r[e].subtitle)
    }
    showMealsPage() {
        this.toggleSections(["search-filters-section", "meal-categories-section", "all-recipes-section"], !0),
        this.toggleSections(["recipe-detail-modal", "nutritional-insights-section", "meal-planning-section", "community-section"], !1),
        this.renderCategories(),
        this.renderRecipeGrid(g.getState().meals),
    ...
    renderCategories() {
        const e = document.getElementById("meal-categories-section");
        if (!e)
            return;
        const t = e.querySelector(".grid");
        if (!t)
            return;
        t.className = "grid grid-cols-6 gap-3";
        const a = g.getState().categories || [];
        t.innerHTML = a.slice(0, 12).map(r => h.createCategoryCard(r)).join("")
    }
    renderRecipeGrid(e) {
        const t = document.querySelector("#all-recipes-section .grid");
        if (!t)
            return;
        if (!e || e.length === 0) {
            t.innerHTML = h.createEmptyState("No recipes found. Try a different search term.");
            return
        }
        t.innerHTML = e.map(r => h.createMealCard(r)).join("");
        const a = document.querySelector("#all-recipes-section p.text-gray-600");
        a && (a.textContent = `Showing ${e.length} recipes`)
    }
    renderAreaFilters() {
        const e = document.querySelector("#search-filters-section .flex.items-center.gap-3");
        if (!e)
            return;
        const t = g.getState().areas || []
          , a = g.getState().selectedArea;
        e.innerHTML = h.createAreaFilters(t.slice(0, 10), a)
    }
    async filterByCategory(e) {
        ...
        t && (t.innerHTML = h.createLoadingSpinner());
        try {
            const a = await x.filterMealsByCategory(e)
              , r = await Promise.all(a.slice(0, 20).map(i => x.getMealById(i.idMeal)));
            g.updateState({
                meals: r.filter(i => i),
                isLoading: !1
            }),
            this.renderRecipeGrid(r.filter(i => i));
            const o = document.querySelector("#all-recipes-section p.text-gray-600");
            o && (o.textContent = `Showing ${r.length} ${e} recipes`)
        } catch (a) {
            console.error("Filter error:", a),
            g.updateState({
                isLoading: !1
            })
        }
    }
    async filterByArea(e) {
        g.updateState({
            selectedArea: e,
            isLoading: !0
        }),
        document.querySelectorAll(".area-filter-btn").forEach(a => {
            a.dataset.area === e ? (a.classList.add("bg-emerald-600", "text-white"),
            a.classList.remove("bg-gray-100", "text-gray-700")) : (a.classList.remove("bg-emerald-600", "text-white"),
        ...
        try {
            let a;
            e ? (a = await x.filterMealsByArea(e),
            a = (await Promise.all(a.slice(0, 20).map(i => x.getMealById(i.idMeal)))).filter(i => i)) : a = await x.searchMealsByName("chicken"),
            g.updateState({
                meals: a,
                isLoading: !1
            }),
            this.renderRecipeGrid(a);
            const r = document.querySelector("#all-recipes-section p.text-gray-600");
            r && (r.textContent = e ? `Showing ${a.length} ${e} recipes` : `Showing ${a.length} recipes`)
        } catch (a) {
            console.error("Filter error:", a),
            g.updateState({
                isLoading: !1
    ...
    async showMealDetail(e) {
        g.updateState({
            selectedMealId: e,
            isLoading: !0
        });
        try {
            const t = await x.getMealById(e);
            if (t) {
                const r = `/meal/${this.slugify(t.strMeal)}`;
                window.location.pathname !== r && window.history.pushState({
                    page: "meal-detail",
                    mealId: e
                }, "", r)
            }
        } catch (t) {
            console.error("Error fetching meal for URL:", t)
        }
        this.renderPage("meal-detail"),
        window.scrollTo({
            top: 0,
    ...
    async showMealDetailPage() {
        var a, r;
        this.toggleSections(["search-filters-section", "featured-recipes-section", "meal-categories-section", "all-recipes-section", "recipe-detail-modal", "nutritional-insights-section", "meal-planning-section", "community-section"], !1);
        let e = document.getElementById("meal-detail-section");
        if (!e) {
            e = document.createElement("section"),
            e.id = "meal-detail-section",
            e.className = "px-8 py-6 bg-gray-50 min-h-screen";
            const o = document.getElementById("main-content")
              , i = document.getElementById("footer");
            o.insertBefore(e, i)
        }
        e.style.display = "";
        const t = g.getState().selectedMealId;
        if (!t) {
            e.innerHTML = `
            ...
            `,
            (a = document.getElementById("back-to-meals-btn")) == null || a.addEventListener("click", () => {
                this.navigateTo("meals")
            }
            );
            return
        ...
        try {
            const o = await x.getMealById(t);
            if (!o)
                throw new Error("Meal not found");
            const i = x.extractIngredients(o)
              , l = x.parseInstructions(o.strInstructions);
            g.updateState({
                selectedMeal: o,
                isLoading: !1
            }),
            e.innerHTML = this.createMealDetailPageContent(o, null, i, l),
            this.setupMealDetailPageListeners(o, i),
            this.loadNutritionData(o, i)
        } catch (o) {
            console.error("Error loading meal detail:", o),
            g.updateState({
                isLoading: !1
            }),
            e.innerHTML = `
            ...
            `,
            (r = document.getElementById("back-to-meals-btn")) == null || r.addEventListener("click", () => {
                this.navigateTo("meals")
            }
            )
        }
    }
    async loadNutritionData(e, t) {
        var r;
        const a = document.getElementById("nutrition-facts-container");
        if (a)
            try {
                const o = t.map(p => `${p.measure} ${p.ingredient}`)
                  , i = await A.analyzeRecipe(e.strMeal, o)
                  , l = A.formatNutritionForDisplay(i)
                  , n = g.getState().mealNutritionCache || {};
                n[e.idMeal] = l,
                g.updateState({
                    mealNutritionCache: n
                }),
                a.innerHTML = this.createNutritionContent(l);
                const d = document.getElementById("hero-calories")
                  , u = document.getElementById("hero-servings");
                d && (d.textContent = `${l.caloriesPerServing} cal/serving`),
                u && (u.textContent = `${l.servings} servings`);
                const m = document.getElementById("log-meal-btn");
                m && (m.disabled = !1,
                m.className = "flex items-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-xl font-semibold hover:bg-blue-700 transition-all cursor-pointer",
                m.title = "",
                m.innerHTML = `
                ...
                console.error("Error loading nutrition data:", o),
                a.innerHTML = `
            ...
            `;
                const i = document.getElementById("hero-calories");
                i && (i.textContent = "N/A");
                const l = document.getElementById("log-meal-btn");
                l && (l.className = "flex items-center gap-2 px-6 py-3 bg-red-100 text-red-500 rounded-xl font-semibold cursor-not-allowed transition-all",
                l.title = 'Nutrition data failed to load. Click "Try Again" in the nutrition section.',
                l.innerHTML = `
                    <i class="fa-solid fa-exclamation-triangle"></i>
                    <span>Unavailable</span>
                `),
                (r = document.getElementById("retry-nutrition-btn")) == null || r.addEventListener("click", () => {
                    a.innerHTML = this.createNutritionLoadingState();
                    const n = document.getElementById("hero-calories");
                    n && (n.textContent = "Calculating...");
                    const d = document.getElementById("log-meal-btn");
                    d && (d.disabled = !0,
                    d.className = "flex items-center gap-2 px-6 py-3 bg-gray-300 text-gray-500 rounded-xl font-semibold cursor-not-allowed transition-all",
                    d.title = "Waiting for nutrition data...",
                    d.innerHTML = `
                        <i class="fa-solid fa-spinner fa-spin"></i>
                        <span>Calculating...</span>
                    `),
                    this.loadNutritionData(e, t)
                }
                )
            }
    }
    ...
    createMealDetailPageContent(e, t, a, r) {
        return `
            <div class="max-w-6xl mx-auto">
                <!-- Back Button -->
                <button id="back-to-meals-btn" class="flex items-center gap-2 text-gray-600 hover:text-emerald-600 font-medium mb-6 transition-colors">
    ...
    setupMealDetailPageListeners(e, t) {
        var a, r, o;
        (a = document.getElementById("back-to-meals-btn")) == null || a.addEventListener("click", () => {
            this.navigateTo("meals")
        }
        ),
        (r = document.getElementById("add-to-plan-detail-btn")) == null || r.addEventListener("click", () => {
            g.updateState({
                selectedMeal: e
            }),
            this.showMealPlanModal(e.idMeal)
        }
        ),
        (o = document.getElementById("log-meal-btn")) == null || o.addEventListener("click", () => {
            this.showLogMealModal(e)
        }
        )
    }
    closeMealDetail() {
    ...
    showNotification(e, t="info") {
        ...
        r.className = `fixed bottom-4 right-4 ${a[t]} text-white px-6 py-3 rounded-lg shadow-lg z-50 toast-notification`,
        r.textContent = e,
    ...
    handleStateChange(e) {}
    showSettingsPage() {
        this.toggleSections(["search-filters-section", "featured-recipes-section", "meal-categories-section", "all-recipes-section", "nutritional-insights-section"], !1),
        this.renderSettingsSection()
    }
    renderSettingsSection() {
        let e = document.getElementById("settings-section");
        if (!e) {
            e = document.createElement("section"),
            e.id = "settings-section",
            e.className = "px-8 py-8 bg-gray-50 min-h-screen";
            const a = document.getElementById("main-content")
              , r = document.getElementById("footer");
            a.insertBefore(e, r)
        }
        e.style.display = "";
        const t = g.getState().userSettings;
        e.innerHTML = `
    ...
    setupSettingsListeners() {
        var e, t;
        document.querySelectorAll(".activity-level-btn").forEach(a => {
            a.addEventListener("click", () => {
                document.querySelectorAll(".activity-level-btn").forEach(r => {
                    r.classList.remove("bg-emerald-600", "text-white"),
                    r.classList.add("bg-gray-100", "text-gray-700")
                }
                ),
                a.classList.add("bg-emerald-600", "text-white"),
                a.classList.remove("bg-gray-100", "text-gray-700")
            }
            )
        }
        ),
        (e = document.getElementById("save-settings-btn")) == null || e.addEventListener("click", () => {
            var r, o, i, l, n, d, u, m, p, f, b;
            const a = {
                age: parseInt((r = document.getElementById("setting-age")) == null ? void 0 : r.value) || 30,
                gender: ((o = document.getElementById("setting-gender")) == null ? void 0 : o.value) || "male",
                weight: parseInt((i = document.getElementById("setting-weight")) == null ? void 0 : i.value) || 70,
                height: parseInt((l = document.getElementById("setting-height")) == null ? void 0 : l.value) || 170,
                calorieGoal: parseInt((n = document.getElementById("setting-calories")) == null ? void 0 : n.value) || 2e3,
                proteinGoal: parseInt((d = document.getElementById("setting-protein")) == null ? void 0 : d.value) || 50,
                carbsGoal: parseInt((u = document.getElementById("setting-carbs")) == null ? void 0 : u.value) || 250,
                fatGoal: parseInt((m = document.getElementById("setting-fat")) == null ? void 0 : m.value) || 65,
                waterGoal: parseInt((p = document.getElementById("setting-water")) == null ? void 0 : p.value) || 2e3,
                waterGlassSize: parseInt((f = document.getElementById("setting-glass")) == null ? void 0 : f.value) || 250,
                activityLevel: ((b = document.querySelector(".activity-level-btn.bg-emerald-600")) == null ? void 0 : b.dataset.level) || "moderate"
            };
            g.updateUserSettings(a),
            this.showNotification("Settings saved successfully!", "success")
        }
        ),
        (t = document.getElementById("reset-data-btn")) == null || t.addEventListener("click", () => {
            confirm("Are you sure you want to reset all data? This cannot be undone.") && (localStorage.clear(),
            window.location.reload())
        }
        )
    }
    showProductsPage() {
        this.toggleSections(["search-filters-section", "featured-recipes-section", "meal-categories-section", "all-recipes-section", "meal-planning-section", "nutritional-insights-section"], !1),
        this.renderProductsSection()
    }
    async renderProductsSection() {
        let e = document.getElementById("products-section");
        if (!e) {
            e = document.createElement("section"),
            e.id = "products-section",
            e.className = "px-8 py-8 bg-gray-50 min-h-screen";
            const a = document.getElementById("main-content")
              , r = document.getElementById("footer");
            a.insertBefore(e, r)
        ...
        e.style.display = "";
        const t = await k.getPopularCategories();
        e.innerHTML = `
    ...
    setupProductsListeners() {
        var e, t, a, r, o;
        (e = document.getElementById("search-product-btn")) == null || e.addEventListener("click", () => {
            var l;
            const i = (l = document.getElementById("product-search-input")) == null ? void 0 : l.value.trim();
            i && this.searchProducts(i)
        }
        ),
        (t = document.getElementById("product-search-input")) == null || t.addEventListener("keypress", i => {
            if (i.key === "Enter") {
                const l = i.target.value.trim();
                l && this.searchProducts(l)
            }
        }
        ),
        (a = document.getElementById("lookup-barcode-btn")) == null || a.addEventListener("click", () => {
            var l;
            const i = (l = document.getElementById("barcode-input")) == null ? void 0 : l.value.trim();
            i && this.lookupBarcode(i)
        }
        ),
        (r = document.getElementById("barcode-input")) == null || r.addEventListener("keypress", i => {
            if (i.key === "Enter") {
                const l = i.target.value.trim();
                l && this.lookupBarcode(l)
            }
        }
        ),
        document.querySelectorAll(".nutri-score-filter").forEach(i => {
            i.addEventListener("click", () => {
                var d;
                document.querySelectorAll(".nutri-score-filter").forEach(u => {
                    u.classList.remove("ring-2", "ring-gray-900")
                }
                ),
                i.classList.add("ring-2", "ring-gray-900");
                const l = i.dataset.grade
                  , n = ((d = document.getElementById("product-search-input")) == null ? void 0 : d.value.trim()) || "";
                n && this.searchProducts(n, l)
            }
            )
        }
        ),
        document.querySelectorAll(".product-category-btn").forEach(i => {
            i.addEventListener("click", () => {
                const l = i.dataset.category;
                this.searchProductsByCategory(l)
            }
            )
        }
        ),
        (o = document.getElementById("products-grid")) == null || o.addEventListener("click", i => {
            const l = i.target.closest(".product-card");
            if (l) {
                const n = l.dataset.barcode;
                this.showProductDetail(n)
            }
        }
        )
    }
    async searchProducts(e, t="") {
        const a = document.getElementById("products-grid")
          , r = document.getElementById("products-loading")
          , o = document.getElementById("products-empty")
          , i = document.getElementById("products-count");
        if (a) {
            ...
            try {
                ...
                t && (l.nutritionGrade = t);
                const n = await k.searchProducts(l);
                r.classList.add("hidden"),
                n.products.length > 0 ? (a.innerHTML = n.products.map(d => h.createProductCard(d)).join(""),
                i.textContent = `Found ${n.count} products for "${e}"`) : (o.classList.remove("hidden"),
                i.textContent = `No products found for "${e}"`),
                g.updateState({
                    searchedProducts: n.products
                })
            } catch (l) {
                console.error("Product search error:", l),
                r.classList.add("hidden"),
                o.classList.remove("hidden"),
                i.textContent = "Error searching products",
                this.showNotification("Failed to search products. Please try again.", "error")
            }
        }
    }
    async searchProductsByCategory(e) {
        const t = document.getElementById("products-grid")
          , a = document.getElementById("products-loading")
          , r = document.getElementById("products-empty")
          , o = document.getElementById("products-count");
        if (t) {
            a.classList.remove("hidden"),
            r.classList.add("hidden"),
            t.innerHTML = "";
            try {
                const i = await k.getProductsByCategory(e);
                a.classList.add("hidden"),
                i.products.length > 0 ? (t.innerHTML = i.products.map(l => h.createProductCard(l)).join(""),
                o.textContent = `Found ${i.count} products in ${e.replace(/_/g, " ")}`) : (r.classList.remove("hidden"),
                o.textContent = `No products found in ${e.replace(/_/g, " ")}`),
                g.updateState({
                    searchedProducts: i.products
                })
            } catch (i) {
                console.error("Category search error:", i),
                a.classList.add("hidden"),
                r.classList.remove("hidden"),
    ...
    async lookupBarcode(e) {
        const t = document.getElementById("products-loading")
          , a = document.getElementById("products-grid")
          , r = document.getElementById("products-empty")
          , o = document.getElementById("products-count");
        ...
        r.classList.add("hidden");
        try {
            const i = await k.getProductByBarcode(e);
            t.classList.add("hidden"),
            i ? (a.innerHTML = h.createProductCard(i),
            o.textContent = `Found product: ${i.name}`,
            g.updateState({
                searchedProducts: [i]
            }),
            this.showProductDetail(e)) : (r.classList.remove("hidden"),
            o.textContent = `No product found with barcode: ${e}`,
            this.showNotification("Product not found in database", "error"))
        } catch (i) {
            console.error("Barcode lookup error:", i),
            t.classList.add("hidden"),
            r.classList.remove("hidden"),
    ...
    async showProductDetail(e) {
        var i, l;
        let t = (i = g.getState().searchedProducts) == null ? void 0 : i.find(n => n.barcode === e);
        if (t || (t = await k.getProductByBarcode(e)),
        !t) {
            this.showNotification("Product not found", "error");
            return
        }
        const a = k.getNutriScoreInfo(t.nutritionGrade)
          , r = k.getNovaGroupInfo(t.novaGroup)
          , o = document.createElement("div");
        o.className = "fixed inset-0 bg-black/50 flex items-center justify-center z-50",
        o.id = "product-detail-modal",
        o.innerHTML = `
        ...
        o.querySelectorAll(".close-product-modal").forEach(n => {
            n.addEventListener("click", () => o.remove())
        }
        ),
        ...
        (l = o.querySelector(".add-product-to-log")) == null || l.addEventListener("click", () => {
            this.logFoodToDaily(t),
            o.remove()
        }
        )
    }
    logFoodToDaily(e) {
        var r, o, i, l;
        const t = g.getTodayString()
          , a = g.getState().dailyLog || {};
        a[t] || (a[t] = {
            totalCalories: 0,
            totalProtein: 0,
            totalCarbs: 0,
        ...
        a[t].totalCalories += Math.round(((r = e.nutrition) == null ? void 0 : r.calories) || 0),
        a[t].totalProtein += Math.round(((o = e.nutrition) == null ? void 0 : o.protein) || 0),
        a[t].totalCarbs += Math.round(((i = e.nutrition) == null ? void 0 : i.carbs) || 0),
        a[t].totalFat += Math.round(((l = e.nutrition) == null ? void 0 : l.fat) || 0),
        a[t].meals.push({
            type: "product",
            name: e.name,
            brand: e.brand,
        ...
        this.showNotification(`${e.name} logged to your daily intake! 📝`, "success"),
        this.updateFoodLogPage()
    }
    showLogMealModal(e) {
        var o, i, l, n, d, u, m, p, f, b, v;
        const t = (o = g.getState().mealNutritionCache) == null ? void 0 : o[e.idMeal]
          , a = document.createElement("div");
        a.className = "fixed inset-0 bg-black/50 flex items-center justify-center z-50",
        a.id = "log-meal-modal",
        a.innerHTML = `
        ...
        const r = a.querySelector("#meal-servings");
        (p = a.querySelector("#decrease-servings")) == null || p.addEventListener("click", () => {
            const y = parseFloat(r.value);
            y > .5 && (r.value = (y - .5).toFixed(1))
        }
        ),
        (f = a.querySelector("#increase-servings")) == null || f.addEventListener("click", () => {
            const y = parseFloat(r.value);
            y < 10 && (r.value = (y + .5).toFixed(1))
        }
        ),
        (b = a.querySelector("#cancel-log-meal")) == null || b.addEventListener("click", () => {
            a.remove()
        }
        ),
        (v = a.querySelector("#confirm-log-meal")) == null || v.addEventListener("click", () => {
            var w;
            const y = parseFloat(r.value) || 1
              , $ = ((w = g.getState().mealNutritionCache) == null ? void 0 : w[e.idMeal]) || t;
            this.logMealToDaily(e, y, $),
            a.remove()
        }
        ),
    ...
    logMealToDaily(e, t, a) {
        var l, n, d, u, m, p;
        const r = g.getTodayString()
          , o = g.getState().dailyLog || {};
        o[r] || (o[r] = {
            totalCalories: 0,
            totalProtein: 0,
            totalCarbs: 0,
        ...
        const i = {
            calories: a ? Math.round(a.caloriesPerServing * t) : 0,
            protein: a ? Math.round((((n = (l = a.macros) == null ? void 0 : l.protein) == null ? void 0 : n.amount) || 0) * t) : 0,
            carbs: a ? Math.round((((u = (d = a.macros) == null ? void 0 : d.carbs) == null ? void 0 : u.amount) || 0) * t) : 0,
            fat: a ? Math.round((((p = (m = a.macros) == null ? void 0 : m.fat) == null ? void 0 : p.amount) || 0) * t) : 0
        };
        o[r].totalCalories += i.calories,
        o[r].totalProtein += i.protein,
        o[r].totalCarbs += i.carbs,
        o[r].totalFat += i.fat,
        o[r].meals.push({
            type: "meal",
            name: e.strMeal,
        ...
        Swal.fire({
            title: "Meal Logged!",
            html: `<p class="text-gray-600">${e.strMeal} (${t} serving${t !== 1 ? "s" : ""}) has been added to your daily log.</p>
                   ${i.calories > 0 ? `<p class="text-emerald-600 font-semibold mt-2">+${i.calories} calories</p>` : ""}`,
            icon: "success",
    ...
    renderFoodLogSection() {
        var o, i;
        let e = document.getElementById("foodlog-section");
        if (!e) {
            e = document.createElement("section"),
            e.id = "foodlog-section",
            e.className = "px-8 py-8 bg-gray-50 min-h-screen";
            const l = document.getElementById("main-content")
              , n = document.getElementById("footer");
            l.insertBefore(e, n)
        }
        e.style.display = "";
        const t = this.getTodayLogSummary()
          , a = this.getWeeklyLogData()
          , r = g.getState().userGoals || {
            dailyCalories: 2e3,
            dailyProtein: 50,
            dailyCarbs: 250,
            dailyFat: 65
                ...
                <div class="bg-linear-to-r from-indigo-600 to-purple-600 rounded-2xl p-6 mb-6 text-white">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="text-right">
                            ...
                            <p class="text-xl font-bold">${new Date().toLocaleDateString("en-US", {
            weekday: "long",
            month: "short",
            day: "numeric"
        })}</p>
    ...
    renderNutritionProgress(e, t, a, r, o) {
        const i = Math.min(Math.round(t / a * 100), 100)
          , l = t > a;
        return `
                    ...
                    <div class="flex items-center justify-between bg-gray-50 rounded-xl p-4 hover:bg-gray-100 transition-all">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-400 mt-1">${new Date(t.loggedAt).toLocaleTimeString("en-US", {
                hour: "numeric",
                minute: "2-digit"
            })}</p>
                            </div>
    ...
    getWeeklyLogData() {
        var r;
        const e = g.getState().dailyLog || {}
          , t = new Date
          , a = [];
        for (let o = 6; o >= 0; o--) {
            const i = new Date(t);
            i.setDate(t.getDate() - o);
            ...
            a.push({
                dayName: i.toLocaleDateString("en-US", {
                    weekday: "short"
                }),
                date: i.getDate(),
                calories: n.totalCalories || 0,
                itemCount: ((r = n.meals) == null ? void 0 : r.length) || 0,
                isToday: o === 0
            })
        }
        return a
    ...
    setupFoodLogListeners() {
        var e;
        (e = document.getElementById("clear-foodlog")) == null || e.addEventListener("click", () => {
            Swal.fire({
                ...
                t.isConfirmed && (this.clearTodayLog(),
                this.renderFoodLogSection(),
                Swal.fire({
                    title: "Cleared!",
        ...
        document.querySelectorAll(".remove-foodlog-item").forEach(t => {
            t.addEventListener("click", () => {
                const a = parseInt(t.dataset.index);
                this.removeLoggedItem(a),
                this.renderFoodLogSection()
            }
            )
        }
    ...
    updateFoodLogPage() {
        const e = document.getElementById("foodlog-section");
        e && e.style.display !== "none" && this.renderFoodLogSection()
    }
    getTodayLogSummary() {
        const e = g.getTodayString();
        return (g.getState().dailyLog || {})[e] || {
            totalCalories: 0,
            totalProtein: 0,
            totalCarbs: 0,
            totalFat: 0,
    ...
    removeLoggedItem(e) {
        ...
        if (!a[t] || !a[t].meals[e])
            return;
        const r = a[t].meals[e];
        a[t].totalCalories -= Math.round(((o = r.nutrition) == null ? void 0 : o.calories) || 0),
        a[t].totalProtein -= Math.round(((i = r.nutrition) == null ? void 0 : i.protein) || 0),
        a[t].totalCarbs -= Math.round(((l = r.nutrition) == null ? void 0 : l.carbs) || 0),
        a[t].totalFat -= Math.round(((n = r.nutrition) == null ? void 0 : n.fat) || 0),
        a[t].totalCalories = Math.max(0, a[t].totalCalories),
        a[t].totalProtein = Math.max(0, a[t].totalProtein),
        a[t].totalCarbs = Math.max(0, a[t].totalCarbs),
        a[t].totalFat = Math.max(0, a[t].totalFat),
        a[t].meals.splice(e, 1),
        g.updateState({
            dailyLog: a
        }, !0),
...
window.addEventListener("load", function() {
    setTimeout( () => {
        try {
            if (typeof Plotly < "u") {
                const s = document.getElementById("macro-chart")
                  , e = document.getElementById("calorie-chart");
                s && !s.data && Ze()
            }
        } catch (s) {
            console.error("Chart rendering error:", s)
        }
    }
    , 1e3)
});
function Ze() {
    try {
        ...
        Plotly.newPlot("macro-chart", s, e, t);
        var a = [{
        ...
        Plotly.newPlot("calorie-chart", a, r, o)
    } catch (i) {
        console.error("Initial chart rendering error:", i)
    }
}
